// 생산요청 등록/수정 모달 + 관련 공유 컴포넌트 (구 영업 부서 입력 화면)
// production-waiting.jsx의 칸반에서 "+ 신규 생산요청"·PENDING 카드 클릭 시 모달로 열린다.
...
/* ════════════════════════════════════════════════════════════
   생산요청 등록/수정 모달 — order가 없으면 신규 다중 등록,
   있으면 PENDING 오더 1건의 모델·용도 수정 + 생산요청 취소
   ════════════════════════════════════════════════════════════ */
function ProductionRequestModal({ order, onClose }) {
  ...
  const [rows, setRows] = useStateSI(() => {
    if (order) {
      return [{
        ...
        usage_type: order.usage_type || '공용',
        qty: 1,
      }];
    }
    return [makeRow()];
  ...
  const isDirty = useMemoSI(() => {
    if (!isEdit || !order || !rows[0]) return false;
    const row = rows[0];
    return ['model_name', 'usage_type'].some(k => (row[k] || '') !== (order[k] || ''));
  }, [isEdit, order, rows]);
  ...
  const submit = () => {
    ...
    if (isEdit) {
      ...
      window.actions.updateOrder(order.order_id, { model_name: row.model_name, usage_type: row.usage_type });
  ...
  const cancelRequest = () => {
    window.actions.showConfirm(
      `오더 #${order.order_id}을(를) 취소할까요?\n이 생산요청은 삭제되며 되돌릴 수 없습니다.`,
      () => { window.actions.cancelOrder(order.order_id); onClose(); },
      { confirmLabel: '생산요청 취소', danger: true }
    );
  };

  return (
    <div className="modal-backdrop" ref={dialogRef}>
      <div className="modal" role="dialog" aria-modal="true" aria-labelledby="prm-title" style={{ width: 860, maxWidth: '96vw', maxHeight: '92vh', display: 'flex', flexDirection: 'column' }}>
        <div className="modal__head">
          <h2 id="prm-title" className="modal__title">
            {isEdit ? `생산요청 수정 · 오더 #${order.order_id}` : '신규 생산요청'}
        ...
        <div className="modal__foot" style={{ flexWrap: 'wrap' }}>
          {isEdit && (
            <button className="btn btn--ghost" style={{ color: 'var(--danger-700)' }} onClick={cancelRequest}>
              <Icon name="x" size={12}/> 생산요청 취소
            </button>
          )}
          {isEdit && (
          ...
          {!isEdit && <button className="btn btn--secondary" onClick={onClose}>닫기</button>}
        </div>
      </div>

      {modal === 'history' && isEdit && (
        <OrderHistoryModal orderId={order.order_id} onClose={() => setModal(null)}/>
      )}
      {modal === 'add-model' && (
        <AddModelModal
            ...
            <div key={h.history_id} style={{ padding: '14px 0', borderBottom: i < history.length - 1 ? '1px solid var(--border-1)' : 'none' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <span className={`badge ${h.action === 'create' ? 'badge--info' : h.action === 'cancel' ? 'badge--danger' : 'badge--pending'}`}>
                    <span className="badge__dot"/>
                    {h.action === 'create' ? '최초 등록' : h.action === 'cancel' ? '취소' : '수정'}
                  </span>
...
window.ProductionRequestModal = ProductionRequestModal;

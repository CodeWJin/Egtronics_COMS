...
→ sales-input.jsx → production-waiting.jsx → ship-inspection.jsx → quality-AwaitPickup.jsx
...
**시리얼 채번 유틸도 `db.js`에 있다** (`SERIAL_MODEL_CODES`, `makeSerialDateCode`, `window.isValidSerialNo`, `window.findModelCodeFromSerial`, `PMDB.generateSerialSuggestion(model, usage, prodDate, excludeOrderId)`). `PMDB.startProduction(order_id)`가 `PENDING → IN_PROGRESS` 전환 직후 이 유틸로 시리얼을 자동 채번해 `saveProduction`으로 즉시 저장한다 — "생산 착수" 버튼을 누르는 순간 시리얼이 이미 부여된다.
...
모델 필터 `<select>`(`production-waiting.jsx`, `quality-AwaitPickup.jsx`)는 옵션 라벨로 `m.description`이 아니라 `m.model`(model_code)을 그대로 표시한다. `sales-input.jsx`의 `ModelSelectModal`은 충전속도(`power`) 칩을 먼저 선택해야 해당 속도의 모델 목록이 나타나며(선택 전에는 빈 안내만 표시), 목록 항목에는 `model_code`와 `description`을 함께 보여준다.
...
**store 주요 필드:**
- `view` — 현재 활성 화면 (`'dashboard' | 'sales' | 'waiting' | 'AwaitPickup' | 'lookup' | 'admin' | 'as-receipt' | 'as-processing'`)
...
| `sales`         | `SalesInputScreen`        | admin, sales (상단 탭 라벨: "생산 요청") |
...
`ROLE_TABS`에 정의되지 않은 역할은 `['lookup']`으로 폴백된다. 모든 역할의 `ROLE_TABS` 배열 맨 앞에 `'dashboard'`가 포함된다. 독립된 "생산 입력" 화면은 없다 — 시리얼·검사 입력은 `waiting`(칸반) 카드 클릭 시 뜨는 모달로 처리한다(아래 "생산 워크플로우" 참고).
...
1. **생산요청** (`sales-input.jsx`, `SalesInputScreen`) — 영업이 모델·용도(공용/비공용)·수량만 입력해 `PENDING` 오더를 생성. 등록 시 `requested_by`에 `currentUser.name`을 저장. 고객사·납품정보 등은 이 단계에서 입력하지 않는다(발주처 등록 필드 자체가 없음).
2. **생산착수** (`production-waiting.jsx`의 칸반, `ProductionEntryModal`) — `PENDING → IN_PROGRESS` 전환(`startProduction`) 시 시리얼이 자동 채번된다. 칸반의 "생산착수" 컬럼 카드를 클릭하면 뜨는 모달에서 생산일자·시리얼(수정/재생성 가능)·검정일자(공용만)·SW/FW버전·기능검사 성적서(`FuncInspectionDrawer`)를 입력하고 제출하면 `completeOrder`로 `AWAIT_PICKUP` 전환.
...
칸반 카드 클릭 동작(`production-waiting.jsx`의 `onPick`)은 컬럼·역할별로 분기한다: 생산요청 카드는 `sales`만 `editOrder`(모델/수량 수정), 생산착수 카드는 `production`/`admin`만 `ProductionEntryModal`, 생산완료 카드는 `sales`/`admin`만 `SalesCompletionModal`, 출하대기 카드는 `AwaitPickup` 뷰 접근 권한이 있는 역할만 `setView('AwaitPickup')`로 이동.
...
### 생산요청 대량 등록 (`sales-input.jsx`)

행마다 모델·용도(공용/비공용)·수량(`row.qty`)만 입력한다. `clampQty(v, max=500)`가 1~500 범위로 정규화하며, 제출 시(`submit()`) 각 행을 `qty`만큼 반복해 `addOrder`를 호출한다(공용/비공용 구분 없이 모두 수량 배수 등록 가능 — 발주처·통신정보 같은 개별 식별자는 이제 생산완료 단계에서 오더별로 입력하므로 생성 시점에는 제약이 없다).
...
- **오더 수정**(`editOrder`)은 `PENDING` 상태에서만 가능하며 모델·용도만 고칠 수 있다(`updateOrder(editing.order_id, { model_name, usage_type })`).
